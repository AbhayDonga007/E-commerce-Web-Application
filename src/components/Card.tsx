...
export function CardProduct() {
  const [data, setData] = useState<Product[]>([]); // Ensuring `data` is always an array
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        setData(res.data); // Correct state update
      } catch (error) {
                  ...
                  <div className="grid relative group">
                    ...
                    >
                      ...
                      <CardHeader className="p-2 absolute z-10 flex-col items-start">
                        <Button isIconOnly className="bg-red-600 text-white font-bold px-2 py-1 rounded-full text-sm">
                      ...
                      <CardFooter className="flex flex-col relative bg-white/30 bottom-0 border-t-1 border-zinc-100/50 z-10">
                        ...
                        <div>
                          <p className="w-auto text-[12px] text-gray-500 text-center font-semibold">
                            {item.type.join(", ")} {/* Ensuring `type` is displayed correctly */}
                          </p>
                        </div>
